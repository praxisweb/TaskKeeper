...
This is a **single-context** repo. There is one domain, one backend, one frontend — not a monorepo.

```
/
├── CONTEXT.md                           ← domain glossary (create with /grill-with-docs)
├── docs/architecture/decisions/         ← ADRs (non-standard path — NOT docs/adr/)
│   ├── 0001-template.md
│   └── ...
└── src/
```

> **Note:** ADRs live at `docs/architecture/decisions/`, not the standard `docs/adr/`.
> Always look there — never `docs/adr/`.

## Before exploring, read these

- **`CONTEXT.md`** at the repo root — domain glossary and bounded context definitions.
